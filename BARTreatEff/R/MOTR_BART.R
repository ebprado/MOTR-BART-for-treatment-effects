                     ...
                     var_linear_pred = NULL) # c('binary treatments', 'covariates + binary treatment', 'covariates + binary treatment + propensity scores')
{
  ...
  if (is.null(x_binary) == FALSE){
    s[binary_treatment_variables - 1] = 0 # these covariates won't used in the tree structure
  }
  ...
  predictions = get_predictions(curr_trees, X, single_tree = ntrees == 1, ancestors, var_linear_pred,
  ...
  for (i in 1:TotIter) {
    ...
    for (j in 1:ntrees) {
      ...
      current_fit = get_predictions(curr_trees[j], X, single_tree = TRUE, ancestors, var_linear_pred,
                                    binary_treatment_variables)
